...
function github_request(string $path, bool $binary = false): string
{
    ...
    // Follow that redirect ourselves so the API token is never sent to the
    // archive host and servers with strict redirect handling can still update.
    for ($redirects = 0; $redirects <= 3; $redirects++) {
        ...
        if ($status >= 300 && $status < 400) {
            if ($redirectUrl === ''
                && preg_match('/^Location:\s*(.+)$/mi', $headersText, $match)) {
                $redirectUrl = trim($match[1]);
            }
            if ($redirectUrl === '') {
                throw new RuntimeException('GitHub negrąžino archyvo peradresavimo adreso');
            }
            ...
            if (!in_array($host, ['api.github.com', 'github.com', 'codeload.github.com'], true)) {
                throw new RuntimeException('GitHub archyvas nukreiptas į neleistiną adresą');
            }
            $url = $redirectUrl;
    ...
    throw new RuntimeException('Per daug GitHub archyvo peradresavimų');
}
